exports.init = async function (homey) {
    ...
    const action_set_filter_cycle = homey.flow.getActionCard('action_set_filter_cycle');
    action_set_filter_cycle.registerRunListener(async (args, state) => {
        await args.device.onCapability_ACTION({filter: parseInt(args.filter), interval: parseInt(args.interval), startTime: args.startTime});
    });

    const action_set_filter_cycle_variable = homey.flow.getActionCard('action_set_filter_cycle_variable');
    action_set_filter_cycle_variable.registerRunListener(async (args, state) => {
        await args.device.onCapability_ACTION({filter: parseInt(args.filter), interval: parseInt(args.interval), startTime: args.startTime});
    });
};