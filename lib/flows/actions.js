exports.init = async function (homey) {
    ...
    action_pump_state.registerRunListener(async (args, state) => {
        await args.device.onCapability_ACTION({'action_pump_state_flow': args.action_on_off});
    });
    ...
    action_light_state.registerRunListener(async (args, state) => {
        await args.device.onCapability_ACTION({'action_light_state_flow': args.action_on_off});
    });
    ...
    action_blower_state.registerRunListener(async (args, state) => {
        await args.device.onCapability_ACTION({'action_blower_state_flow': args.action_on_off});
    });
    ...
    action_heater_mode.registerRunListener(async (args, state) => {
        await args.device.onCapability_ACTION({'action_heater_mode_flow': args.action_on_off});
    });
};