...
class ControlMySpa {
    ...
    isLoggedIn() {
        return !!this.tokenData.access_token && this.tokenData.time + this.tokenData.expires_in > ((new Date() / 1e3) | 0) ? true : false;
    }
    ...
    async login() {
        try {
            ...
            if (requestValid) {
                const body = req.data;
                this.tokenData = { ...body, time: (new Date() / 1e3) | 0 };
    ...
    async getSpa() {
        try {
            ...
            if (this.spaId && !this.currentSpa) {
                this.spaId = this.spaId;
                console.log('Set spaId from this.spaId', this.spaId);
            } else if (this.currentSpa) {
                this.spaId = this.currentSpa._id;
                console.log('Set spaId from currentSpa', this.spaId);
            }

            const req = await this.instance.get(`https://iot.controlmyspa.com/spas/${this.spaId}`, {
                headers: {
                    Authorization: 'Bearer ' + this.tokenData.access_token
                }
            });
