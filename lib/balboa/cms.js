...
class ControlMySpa {
    ...
    isLoggedIn() {
        return !!this.tokenData.access_token && this.tokenData.timestamp + ((this.tokenData.expires_in + 100) * 1000) > Date.now() ? true : false;
    }
    ...
    async login() {
        try {
            ...
            if (requestValid) {
                const body = req.data;
                this.tokenData = { ...body, timestamp: Date.now()};
    ...
    async getSpa() {
        try {
            ...
            if (requestValid) {
                ...
                const spa = body._embedded && body._embedded.spas && body._embedded.spas.length > 0 ? body._embedded.spas[0] : {};
